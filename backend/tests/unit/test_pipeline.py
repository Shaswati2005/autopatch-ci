...
async def test_log_parser_extracts_error_info():
    ...
    event = CIFailureEvent(
        ...
        branch="main",
        run_id="run-101",
        raw_log="FAILED src/calculator.py::test_calc - TypeError: unsupported operand",
    )
    ...
    assert "TypeError" in result.error_summary or "TypeError" in result.error_type
...
async def test_gemini_patcher_generates_code_and_test():
    ...
    patcher = GeminiLLMPatcherAdapter(api_key="")
